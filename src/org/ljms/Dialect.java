...
/**
 * The entire portability surface of LJMS: one expression.
 *
 * Everything else in {@link QueueDAO} is plain SQL that runs unchanged on any
 * database with transactions — SELECT, UPDATE, INSERT, CURRENT_TIMESTAMP.
 * The only thing the standard leaves to vendors is date arithmetic, so it
 * lives here and nowhere else. Porting LJMS to a database not listed is one
 * line, not an abstraction layer.
 *
